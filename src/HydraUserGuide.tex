...
\label{sec:org3d45a45}

\emph{DefsChapter.org is loaded}
\chapter*{Hydra User Guide}
\label{sec:orgc27805b}
...
\label{sec:org3f6a8f7}

To design a circuit, and to do anything useful with it, we need a
way to describe it.  The description can take two forms.  A
\emph{specification} is a clear statement of what the system is
intended to do, while an \emph{implementation} shows how the primitive
...
specification uses a \emph{hardware description language}.  Both forms
...
Hydra is an example of an \emph{embedded domain-specific language} (DSL
...
programming language.  It is implemented by \emph{embedding} in
...
\section*{Install and test}
\label{sec:org4ce4972}
...
\url{https://www.haskell.org/downloads/}.  To check that ghc is
working, enter
...
\texttt{cscode} directory, which is available as a zip file.  You don't
need to install anything in \texttt{cscode}: it can be used without any
further installation or preparation.  Please don't delete any of
the files or directories in \texttt{cscode}, as that could prevent some
of the circuits from working.

A collection of examples can be found in this directory:\\
\texttt{cscode/circuits/HDL/Hydra/Examples}

To run circuit simulations, you always need to be in the
\texttt{circuits} directory.  If you're in some other directory, it
won't work.

Here is an example of a very simple circuit, which consists of
just a couple of logic gates.  Thie file is
\texttt{circuits/HDL/Hydra/Examples/SimpleCirc.hs}.  This example is
...
\texttt{circuits/HDL/Hydra/Examples/SimpleCircRun.hs}.  The following
commands will run a simulation of the circuit, using input data
provided in the driver.  As always, you must be in the \texttt{circuits}
directory.
...
\label{sec:org3ee6b4a}

A data value in a circuit is called a \textbf{signal}.  A signal is
...
\subsection*{Logic gates and flip flops}
\label{sec:orgadc5511}
...
\includegraphics{../../figures/tikz/yeqinvx.pdf}
...
\includegraphics{../../figures/tikz/xeq-and2ab.pdf}
...
nor4 a zero c d
\end{verbatim}
\subsection*{Set of logic gates}
\label{sec:orgddf66c4}
...
to input signals named \texttt{a}, \texttt{b}, etc., and produces one output
signal.
...
\label{sec:org60f8768}

A constant signal always carries the same value: either it is
always 0, or always 1.  The names of these two constant signals
are written as \texttt{zero} and \texttt{one}.  Signal names in Hydra always
begin with a lower case letter, never with a digit.  To denote a
constant signal in a circuit specification, write either \texttt{zero} or
\texttt{one}.  Don't use 0/1, or T/F, or True/False in a circuit
...
\end{verbatim}
\subsection*{Named signals}
\label{sec:org27fec5c}
...
nand3 x (and2 p q) z
\end{verbatim}
\subsection*{Producing several outputs}
\label{sec:orgb0b63ff}
...
\begin{verbatim}
module HDL.Hydra.Examples.HalfAdd where
...
\item A device driver, which converts between human readable input and
...
\begin{verbatim}
module HDL.Hydra.Examples.HalfAddRun where
-- Device driver for half adder

import HDL.Hydra.Core.Lib
import HDL.Hydra.Examples.HalfAdd
...
\end{itemize}
\section*{Basic circuit simulation}
\label{sec:org898d6b5}
\subsection*{Simple device driver}
\label{sec:org305fc26}
\subsection*{Combinational simulation}
\label{sec:orga608fe1}
...
\begin{verbatim}
module HDL.Hydra.Examples.Mux1 where
import HDL.Hydra.Core.Lib
...
\begin{verbatim}
module HDL.Hydra.Examples.Reg1 where
import HDL.Hydra.Core.Lib
import HDL.Hydra.Examples.Mux1
...
\end{verbatim}
\subsection*{Example: register}
\label{sec:org8aebf24}
\section*{Modules and files}
\label{sec:org193f16a}
...
module HDL.Hydra.Examples.SimpleCirc where
import HDL.Hydra.Core.Lib
...
\label{sec:orge8e9997}
\section*{Circuit simulation}
\label{sec:orge7f4634}
...
\item \texttt{SimpleCircRun.hs} defines a device driver for the circuit.  The
file provides a main program named \texttt{main} which runs the
...
\subsection*{Basic device drivers}
\label{sec:org867a288}
...
ends in "Run".
\subsection*{The device driver}
\label{sec:orgbec4f50}
...
module Main where
import HDL.Hydra.Core.Lib
import HDL.Hydra.Examples.SimpleCirc
\end{verbatim}
...
input prt named in\textsubscript{x}.  Each input port is created with a statement
of the form portname <- inPortBit "signame".  If the inport is a
word, rather than a bit, you would definit like this: in\textsubscript{w} <-
...
\end{verbatim}
\subsection*{Drivers}
\label{sec:org3bca2c0}
\subsection*{Automatic output}
\label{sec:org41a393a}

\subsection*{Feedback and changing state: BSR4}
\label{sec:orgff15298}
...
op=00 l=0 r=0 x=0   Output lo=1 ro=1 y=9
\end{verbatim}
\subsection*{Synchronous sequential simulation}
\label{sec:org90d5a5b}
...
"\ldots{}" says that the literal string "\ldots{}" should appear in the
...
\subsection*{SimDriverRun.hs}
\label{sec:org3364207}
...
-- Run the circuit
  runCircuit
\end{verbatim}
\subsection*{Show Version.hs}
\label{sec:org20050f6}
...
\section*{Hydra Syntax}
\label{sec:org1f6a184}
...
\subsection*{Modules}
\label{sec:orge53883d}

\subsection*{Names}
\label{sec:org837526c}
...
signal:
\subsection*{Scope}
\label{sec:org0fd6a19}

\subsection*{Signal expressions}
\label{sec:org40bdabb}
...
suppose we have signals named \textbf{x} and \textbf{y}.  Then the following
expressions denote the corresponding signals.  Note that \textbf{zero}
and \textbf{one} are simply the names of the constant signals.
...
\item An application of a circuit to inputs denotes a signal: \textbf{or2 x
y} specifies a signal which is the output of an \textbf{or2} gate
connected to inputs \textbf{x} and \textbf{y}.
\end{itemize}
...
\end{verbatim}
\subsection*{Comments}
\label{sec:org348a443}
There are two ways to indicate a comment
\begin{itemize}
\item Enclose the comment in brackets \{- so this is a comment -\}
\item A double dash -- indicates that everything else -- on the line
is a comment
\end{itemize}
\subsection*{Indentation}
\label{sec:org9740098}
...
circ x y = a  -- a bad definition
  where
   ...
   q = bla...
    r = bla...
\end{verbatim}
\subsection*{Pitfalls and errors}
\label{sec:org0e654e5}
\subsection*{Errors - ConstantSignals.hs}
\label{sec:org40c5109}

-- ConstantSignals: illustrate right and wrong way to define constant
-- This file is part of Hydra. See README and \url{https://github.com/jtod/Hydra}
...
\subsubsection*{Circuits with several outputs}
\label{sec:org373703d}
...
\end{verbatim}
\subsection*{Words}
\label{sec:orgc0138bf}

\begin{itemize}
\item A word \([x_0, x_1, \ldots, x_{k-1}]\) is used to represent binary
numbers.
...
\item Sometimes you need to extract a bit or a field from a word, or
construct new words.

\item Therefore we need operators to add a new bit to a word, extract
a bit from a word, etc.
\end{itemize}
\subsubsection*{Words}
\label{sec:orgaba0ee8}
...
\subsubsection*{List notation for words}
\label{sec:org1af0a3a}
...
\label{sec:orgc72d8ce}
\subsubsection*{Building words}
\label{sec:org068ebb6}
...
very useful.
\subsubsection*{Accessing parts of words with a pattern}
\label{sec:orgeb4ac30}
...
[a,b,c,d] = w
\end{verbatim}
\subsubsection*{Accessing parts of a word}
\label{sec:org4acf765}
...
no matter how large.
\subsubsection*{Big Endian and Little Endian}
\label{sec:org78561a5}
...
\end{itemize}
\subsubsection*{Indexing an element of a word}
\label{sec:org10e8f7d}
...
\label{sec:orgfbebb49}
\subsubsection*{Using list pattern notation}
\label{sec:orga6b45bb}
...
a word (the sum).
\begin{itemize}
\item The type |[(a,a)]| is an important special case
...
\subsection*{A circuit with words and internal signals}
\label{sec:orgdd86c4f}
...
\label{sec:orgea42892}

The \textbf{type} of a value determines what operations you can perform
...
figure out \textbf{where} the error is.
...
they always contain the symbol \textbf{::}, and usually contain some
arrows \textbf{=>} and \textbf{->}.  A typical example is
...
\label{sec:org3477482}

The interface gives the name of the circuit and names its inputs
and outputs.  A circuit is created with a \textbf{circuit defining
...
inputs named \textbf{input1} and \textbf{input2}, and produces an output with
...
\textbf{mycirc}, and they can be used to calculate the value of the
output.  Another circuit can connect signals with arbitrary names,
or no names at all, to the inputs of \textbf{mycirc}.
\subsection*{Interfaces and Types}
\label{sec:orgd4a793c}
...
\label{sec:orgd0d1693}

\textbf{Short version.} If you're writing a routine circuit and just want
to simulate it, you can just write \textbf{CBit a =>} for the signal
class constraint and then use \textbf{a} as the type for every bit
...
specific type.  Many types can be used, for example \textbf{Bool} or
\textbf{Stream Lattice}.  The choice of type determines what happens
...
\textbf{a} rather than \textbf{Bool} as the bit signal type.
\subsection*{Inputs and outputs}
\label{sec:org2f05af4}

After the signal class (i.e. after the \textbf{=>} symbol) come the types
of the inputs and output of the circuit.  In the simplest case,
each input or output signal is just a bit of type \textbf{a}.  There may
be any number of input arguments, and there must be one output
result.  A single arrow \textbf{->} must follow each input; thus the
number of single arrows in the type is the same as the number of
inputs.

The inverter has one input of type \textbf{a}, which is followed by \textbf{->},
and the type \textbf{a} of the output appears last.  The type declaration
...
function \textbf{im} that is given a complex number (type \(C\)) and
...
input is followed by \textbf{->}.  To find out how many inputs a circuit
takes, just count the number of times \textbf{->} appears in its type.
...
\label{sec:orge2993c4}
\subsection*{Combinational signals: Bit a}
\label{sec:org7edba9c}
...
You can do these other things with \textbf{halfAdd}, but not with
\textbf{halfAddB}.
...
\textbf{Bit} is used for combinational circuits, and \textbf{CBit} ("clocked
bit") is used for sequential circuits.

signal.  The notation \textbf{Bit a =>} means that \textbf{a} can be any type in
the set \textbf{Bit}, and therefore all of the Bit operations can be
performed on a signal of type \textbf{a}.
...
as logic gates.
\end{itemize}
\subsection*{Clocked signals: CBit a}
\label{sec:org6dda2a5}
...
\label{sec:org0327f80}

\subsection*{Form of a type declaration}
\label{sec:org1f76b9e}
...
\item input and output types: a for a bit signal, [a] or (a,a) for a
container holding several bits
\end{itemize}
\subsection*{Type declaration is optional}
\label{sec:org980e00c}

\begin{itemize}
\item You can omit the type declaration; the compiler will work it out
automatically
...
\item For this reason, it's not a good idea to give circuit types like
\texttt{inv :: Bool -> Bool}.
\item Instead, we use a \emph{type variable}, typically \texttt{a}, to
represent the signal type.
\end{itemize}
\subsection*{The signal class constraint}
\label{sec:org5a60f6b}
...
\label{sec:orgf683c1c}

\begin{itemize}
\item By convention, the type of a bit signal is called \texttt{a}.
\item The scope of the type variable \texttt{a} is the type statement.
\item This means you can have a signal named \texttt{a} in the circuit
...
\label{sec:org8970c21}

\begin{itemize}
\item For a combinational circuit, use |Bit a =>| and the type of a
bit signal is |a|
\item For a clocked synchronous circuit, use |CBit a =>| and the type
...
\begin{itemize}
\item You can use |winv4 x| if |x| is a 4-bit word, but
not if it's an 8 or 16 bit word.
\item The generic circuit |winv| works for any word
...
\item This is not software to do the operation --- it is
still a real circuit.
\end{itemize}
\subsection*{Calculating the and/or of many bits}
\label{sec:org9af07d7}
...
mux1w c x y = map2 (mux1 c) x y
\end{verbatim}
\subsection*{Sized map}
\label{sec:orgdfa3908}
...
the word.
\subsection*{Fold from the left}
\label{sec:orge929dc5}
...
\end{itemize}
\subsection*{Binary comparison using foldl}
\label{sec:org893873a}
...
\end{verbatim}
\subsection*{Fold from the right: foldr}
\label{sec:org77656b5}
...
\section*{Standard circuits for bits}
\label{sec:org86f399e}

\subsection*{Constant signals}
\label{sec:org4abbf9e}
...
\end{verbatim}
\section*{Standard circuits for words}
\label{sec:org2d90ab1}
...
\label{sec:org3822119}

\subsection*{Combinational shifting}
\label{sec:org377c47d}
...
shl x = [x!!i | i <- [1..k-1]] ++ [zero]
  where k = length x
\end{verbatim}
\subsection*{Bit slice representation}
\label{sec:org5e97d09}
...
\label{sec:orgd4df35c}

\subsection*{Multiplexers}
\label{sec:org1ebac63}
...
\label{sec:org85bf62c}

\subsection*{Binary addition}
\label{sec:org4fab090}
...
\end{verbatim}
\section*{Deprecated and old stuff}
\label{sec:org03add48}
