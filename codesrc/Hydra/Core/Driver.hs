-- Driver: device drivers for Hydra circuits
...
{-|
Module       : Hydra.Core.SimDriver
Description  : Tools for writing device drivers
Copyright    : (c) 2022 John T. O'Donnell
License      : GPL-3
Maintainer   : john.t.odonnell9@gmail.com
Stability    : experimental

Tools for writing device drivers for circuits specified in Hydra -}
...
-- The device driver maintains its own state, consisting of a pair
...
-- Default commands.  The device driver can define additional
...
--  printLine "clockCycle..."
  ...
  liftIO $ putStrLn (take 65(repeat '-'))
...
-- The output section of a device driver uses format specifiers to
...
runCircuit :: StateT (SysState a) IO ()
runCircuit = do
...
-- The input section of a device driver is constructed using
...
-- inputs.   Different device drivers will take different
-- approaches to this.  A set of functions is defined here, and the
-- device driver can use any of them.
